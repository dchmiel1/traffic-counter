...
class STrack(BaseTrack):
    ...
    def check_if_disappearing(self, det):
        ...
        if (mean_width * 0.85 > width and is_slower_than_expected(x_center, x_last_center, mean_vel_x)):
            is_disappearing = True

        if (mean_height * 0.85 > height and is_slower_than_expected(y_center, y_last_center, mean_vel_y)):
            is_disappearing = True
    ...
    def modify_prediction(self):
        ...
        if abs(valid_mean_vel_x) > 1:
            if is_right:
                det[2] = det[0] + valid_mean_width * 1.2
                det[0] = det[2] - mean_width
            else:
                det[0] = det[2] - valid_mean_width * 1.2
                det[2] = det[0] + mean_width

        if abs(valid_mean_vel_y) > 1:
            if is_down:
                det[3] = det[1] + valid_mean_height * 1.2
                det[1] = det[3] - mean_height
            else:
                det[1] = det[3] - valid_mean_height * 1.2
    ...
    def update(self, new_track, frame_id):
        ...
        self.last_size.append((self.xyxy[2] - self.xyxy[0], self.xyxy[3]- self.xyxy[1]))
